...
plot_qspeed_speed <- function(vbv_data) {
  vbv_data %>%
  ggplot() + 
  geom_point(aes(qspeed, speed, color = vehicle_type),
             alpha = 0.5) +
  geom_vline(aes(xintercept = 25)) +
  scale_color_discrete() +
  facet_grid(cols = vars(lane)) +
...
plot_length_speed <- function(vbv_data) {
  vbv_data %>%
  ggplot() + 
  geom_point(aes(length, speed, color = vehicle_type),
             alpha = 0.5) +
  geom_hline(aes(yintercept = 1)) +
  geom_hline(aes(yintercept = 5.6), color = "grey") +
  geom_hline(aes(yintercept = 27)) +
...
For **LM** gjelder følgende:
  ...
  - Fart og lengde er ugyldig dersom:
    - ::: {style="color:green"}
      Fartskvalitet* \>= 25 % for fart \>= 100 km/h, eller \>= 25 km/h for fart \< 100 km/h. OBS! Endret mars 2020 fra 3 % / 3 km/h.
      :::

 - Klassifisering er ugyldig dersom:
    - Signalkoden inneholder oddetall eller 0, unntatt 0202.
    
\*Fartskvalitet fra LM er det absolutte avviket (i km/h) mellom sløyfe 1 og 2.
...
For **EMU3** gjelder følgende:
  ...
  - Fart, lengde og klassifisering er ugyldig dersom:
    - ::: {style="color:red"}
      Fartskvalitet ≠ 0 (omsatt fra "Change of speed validity bit" satt til TRUE).
      :::
...
Lavere datakvalitet oppstår av ulike årsaker som grovt kan deles inn i tre kategorier:
  ...
  - Høy akselerasjon, typisk på ramper (fartskvalitet)
...
For å gjennomføre analysen, gjøres følgende:
  ...
  5. Se på et utvalg av data for ett kjørefelt og finn eventuelle sammenhenger mellom lengde, klasse, fart og fartskvalitet
...
## Delanalyser
  ...
  9. Fartskvalitet over 3 eller ulik 0: lengde ok?
  10. Fartskvalitet over 3 eller ulik 0: klasse ok?
...
#### Ev 39 Øysand F05
På teststasjon F05 har vi tilfellet med ulik frekvens på sløyfene i felt 2. Frekvensen ligger på omkring 65 og 85 kHz. Felt 2 har stort sett qspeed over 3 og dermed underkjente lengdemålinger.
...
Vurdering:
  ...
  - I felt 4 får de fleste kjøretøy omtrent maksimal fartskvalitet, dvs. at den ene fartsmålingen er nær null uansett.
  ...
  - Fartskvalitet ser her ut til å være en god indiaktor på lav datakvalitet for lengde og klassifisering.
...
Vurdering:
  ...
  - Noen lave hastigheter gir bestemte feilverdier for lengde, satt av LM. Disse er utenfor vårt toleranseområde for lengde, men har ikke særlig høy fartskvalitet.

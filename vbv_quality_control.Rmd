...
Hele 1. desember 2021 var det en del snøfall gjennom hele døgnet. Derfor er det ikke forventet en eneste motorsykkel i datagrunnlaget, men likevel er det ganske mange forekomster. Dette skyldes feltskifte og det typiske er at ett reellt kjøretøy blir registrert som to. Som oftest får begge korte lengder, hvorav ett nesten alltid blir feilaktig klassifisert som motorsykkel.

## Kø på Rv 706 Havnegata
Trafikkregistreringspunktet på Rv 706 Havnegata ligger nær en rundkjøring. Det er fire felt og en del feltskifte over sløyfene. Når det er mye trafikk vil en del kjøretøy passere sløyfene med veldig lav fart og tidvis inntreffer helt stillestående trafikk.

Ved å se på videoopptak og gå gjennom data kjøretøy for kjøretøy er det utarbeidet en fasit for lengdeklassifisering. En sammenligning er gitt i tabellen nedenfor.

<\br>

```{r contingency_lm_length_vs_true_vehicle}
# short_lengths_lm <-
#   short_lengths %>% 
#   dplyr::filter(
#     datalogger_type == "LM"
#   )
#   
# contingency_lm_lengths_vs_true_vehicle_1 <-
#   table(
#     short_lengths_lm$length, 
#     short_lengths_lm$true_vehicle
#   ) %>% 
#   addmargins()
# 
# contingency_lm_lengths_vs_true_vehicle <-
#   short_lengths %>% 
#   dplyr::filter(
#     datalogger_type == "LM"
#   ) %>% 
...
Selv om lengdemålingene gir for korte lengder, er det likevel faktiske kjøretøy.

```{r contingency_lm_length_vs_true_length_class}
short_lengths %>% 
  dplyr::filter(
    datalogger_type == "LM"
  ) %>%
...
Det er få tunge kjøretøy som kjører sakte over sløyfene i datamaterialet, så det kan ikke sies at det kun er korrekt at alle med kort lengde blir riktig lengdeklassifisert. 

Ved lav fart der mange kjører med liten fysisk luke, vil det føre til sammenslåing av kjøretøy. De sammenslåtte vil i mange tilfeller se ut som tunge kjøretøy i dataene. Det blir da to feil i data, nemlig en for lav trafikkmengde og en for høy andel tunge.
...
Hele 1. desember 2021 var det en del snøfall gjennom hele døgnet. Derfor er det ikke forventet en eneste motorsykkel i datagrunnlaget, men likevel er det noen forekomster. Dette skyldes nok hovedsaklig feltskifte eller saktegående trafikk. Det typiske er at ett reellt kjøretøy blir registrert som to, og som oftest får begge korte lengder, hvorav ett nesten alltid blir feilaktig klassifisert som motorsykkel.
...
```{r contingency_emu3_length_vs_true_vehicle}
short_lengths %>% 
  dplyr::filter(
    datalogger_type == "EMU3"
  ) %>%
  flextable::proc_freq(
    "length",
    "true_vehicle"
...
Alle registreringer med korte lengder i dette datautvalget var reelle kjøretøy.

```{r contingency_emu3_length_vs_true_length_class}
short_lengths %>% 
  dplyr::filter(
    datalogger_type == "EMU3"
  ) %>%
...
Ved lav fart der mange kjører med liten fysisk luke, men det er veldig få tilfeller der to kjøretøy blir slått sammen til en registrering. EMU3 virker ganske tolerant for små luker.

Lengdemålingene blir tydelig påvirket av endring i fart over sløyfene. Dette er lett å observere når farten er relativt lav. Det er gjentakende observert at ved oppbremsing blir lengdene for lange, og ved akselerasjon blir de for korte. 

Stillestående kjøretøy blir registrert, men i etterkant kan det virke som om datalogger kommer "ut av rytmen" og overser flere kjøretøy.

Ved feltskifte er det få tilfeller av duplikater.
...
Der kjøretøy skifter felt over sløyfene vil det i mange tilfeller bli målt for kort lengde og gitt feil klasse, typisk vil dette føre til falske motorsykler. Det vil i mange tilfeller også oppstå duplikater hvor minst en av registreringene får veldig kort lengde.
...
## Saktegående trafikk
Ved saktegående trafikk vil lengdemålingene bli veldig usikre. Når kjøretøy står stille på sløyfene og det påfølgende kjøretøy kommer veldig tett på, kan det oppstå sammenslåing av kjøretøy. For eksempel kan to personbiler bli tolket som ett langt kjøretøy både i form av lengde og klasse. Noen slike sammenslåtte registreringer ser ut som et autentisk langt kjøretøy, men noen får lengde over 27 m og/eller ukjent klasse.
...
Gjenstående analyser av enkeltkjøretøyregistreringer med videoopptak:

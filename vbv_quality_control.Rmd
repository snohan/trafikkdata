---
title: "Vurdering av regler for kontroll av trafikkdata"
output: 
  html_notebook:
    theme: cerulean
...
# Kvalitetskontroll av enkeltkjøretøyregistreringer
...
Reglene ble utarbeidet i samråd med dataloggerleverandørene, samt basert på egen testing der fasiten er laget på grunnlag av manuell gjennomgang av videoopptak kombinert med kjøretøyopplysninger fra Kjøretøyregisteret.

Reglene for kvalitetskontroll er inndelt i to hovedkategorier:

-   ett sett som gjelder alle data, og

-   ett sett som gjelder spesifikt per dataloggertype.

### Regler for alle utstyrstyper
...
Det antas at kvaliteten på fartsmålingene henger nøye sammen med kvaliteten på lengdemålingene. Det er kjøretøylengden vi kan finne fasit på i kjøretøyregisteret, så derfor er lengden i fokus for vurderingen. Det samme gjelder også kjøretøyklassifiseringen.

Hva er grunnene til at lengdemålingene blir underkjente?

Målet er å vurdere dagens regler og eventuelt justere dem. Om reglene justeres, skal alle data i Trafikkdatasystemet reberegnes tilbake til 2014.
...
Problem: regelen setter alle lengder ugyldig siden qspeed er "nøll", og den er jo nettopp det for tidligere fw-versjoner. HMmm. Er den delen av regelen utilsiktet? Vi har jo ikke full fw-historikk, så spørsmålet er om vi bare skal være strenge med data fra nyeste versjonene?
...
Dagens regler inneholder ingen firmwarespesfikke regler, men de reviderte må ta hensyn til dette.
...
Uke 23, 2021, mandag 7. juni - onsdag 9. juni: to LM og en EMU3 - har de samme lengder?
I felt 2 på F05 har en av sløyfene lavere frekvens enn normalt, og dette gir utslag i større qspeed og noe lengre lengder enn de samme målingene i felt 2 på F09.

```{r gof}
# old:
# f09 <- oysand_2021w23 %>%
#   dplyr::filter(traffic_registration_point_id == "23827V72241") %>%
#   dplyr::select(length)
#
# f07 <- oysand_2021w23 %>%
#   dplyr::filter(traffic_registration_point_id == "23679V72241") %>%
#   dplyr::select(length)
#
# f05_1 <- oysand_2021w23 %>%
#   dplyr::filter(traffic_registration_point_id == "23531V72241",
#                 lane == 2) %>%
#   dplyr::select(length)
#
# f09_1 <- oysand_2021w23 %>%
#   dplyr::filter(traffic_registration_point_id == "23827V72241",
#                 lane == 2) %>%
#   dplyr::select(length)
...
oysand_2021w23_agg_length_class_full %>% 
  dplyr::filter(weekday == "mandag") %>% 
  ggplot(aes(length_class_full, volume, fill = name_and_datalogger)) +
...
oysand_2021w23_agg_length_class_2 %>% 
  dplyr::filter(weekday == "mandag") %>% 
  ggplot(aes(length_class_2, volume, fill = name_and_datalogger)) +
  ggplot2::geom_col(position = "dodge") +
  facet_grid(rows = vars(lane)) +
  theme_minimal() +
  scale_fill_viridis_d(name = "Datalogger",
...
Sammenligner kjøretøy for kjøretøy med fasit på lengde fra kjøretøyregisteret.
...
-   qspeed-regel gjelder kun for fw med konsistent verdi for qspeed (s2.17.4 og 1.0x)

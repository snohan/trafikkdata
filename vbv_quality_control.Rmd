...
```{temp r}
# Fordeling av lengder (3, 7,6)
# geom_density is smoothing, but does it assume normality?

# To make it easier to compare distributions with very different counts,
# put density on the y axis instead of the default count
ggplot(diamonds, aes(price, after_stat(density), colour = cut)) +
  geom_freqpoly(binwidth = 500) # samme som histogram, bare med punkt i toppen og linje mellom punktene
# kan dette funke med bin = 0.1 m? blir sikkert veldig hakkete...
```
...
Se om måleverdiene har endret seg med firmware:
...
Se om måleverdiene har endret seg ved bytte av datalogger (samme sløyfer):

  - Elgeseter gate ved Abels gate (kø) bare veldig gammel EMU-fw
  - Garli (sentrisk)
  - Heimdalsmyra (feltskifte?)
...
**Klassifisering basert på lengde eller klasse**:
  ...
  - Gjøre egne tester på dette - benytt testing for ny rammeavtale.

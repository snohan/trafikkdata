  ...
  pdf:
    ...
    include-in-header:
      text:
        ...
        \newfontfamily\subsubsectionfont[Color=444f55]{Arial}
...
Klassifisering av motorkjøretøy kan gjøres på ulike måter avhengig av hvilket formål det skal tjene. Dette dokumentet tar for seg sammenhengen mellom klassene i følgende klassifiseringssystemer:
...
Videre viser en analyse av sammenhengen mellom kjøretøylengde og tillatt totalvekt at skillet mellom korte og lange kjøretøy burde vært ved 7,5 m dersom grensen skal passe best til skillet mellom lette og tunge ved 3,5 tonn tillatt totalvekt. Likevel er forskjellen fra den tradisjonelle lengdegrensen på 5,6 m relativt liten, og ikke av særlig betydning i de fleste sammenhenger.

Til slutt er det sett på sammenhengen mellom alle klassifiseringssystemene for å se hvilke klasser som tilsvarer hverandre. I trafikkregistreringer vil hvert kjøretøy bli forsøkt klassifisert ut fra de data som trafikkregistreringsutstyret klarer å samle inn via sensorikk. Hvilke klasser som benyttes i slike registreringer kan variere mellom utstyrsleverandører. Dette dokumentet beskriver sammenhengen mellom slike utstyrsspesifikke klassifiseringer og de ovenfor nevnte klassifiseringssystemene. Dersom NorSIKT-systemet skal benyttes, viser det seg å være nivå 3 som passer best med data fra dagens trafikkregistreringsutstyr som benyttes i Trafikkdatasystemet.


# Klassifiseringssytemer
I trafikkdatasammenheng har det i mange år vært vanlig å klassifisere motorkjøretøy basert på *målt* lengde. En slik lengdemåling foretas automatisk når et kjøretøy blir registrert av sensorer idet det passerer et trafikkregistreringspunkt. Slike trafikkregistreringspunkt er plassert på offentlig veg og foretar registrering av alle passerende kjøretøy uten at det påvirker trafikken.
...
Nivå 1 har to klasser:
  ...
  - korte: kortere enn 5,6 m
  - lange: 5,6 m og lengre
...
Tidligere var motorsykler angitt som egen klasse, men ble senere inkludert i klassen "kortere enn 5,6 m".
...
## Nordisk klassifiseringsstandard (NorSIKT)
De nordiske landene har samarbeidet om et felles klassifiseringssystem, nordisk system for intelligent klassifisering av trafikk (NorSIKT). Systemet omfatter 6 nivåer med et ulikt antall klasser i hvert nivå. Nivå 1 er triviell og har kun én klasse for motorkjøretøy - enten er kjøretøyet et motorkjøretøy eller ikke.
...
NorSIKT nivå 2 har følgende fem klasser:
  ...
  - OMV: other motor vehicle (andre motorkjøretøy)
...
Kjøretøyklassifiseringen fra trafikkregistreringsutstyret er basert på mer enn bare lengdemåling. Hvilke måleverdier som inngår i klassifiseringen er avhengig av sensortype og utstyrsleverandør.


### Kontinuerlige registreringer
I perioden fra 2015 til 2018 ble det innfaset nytt registreringutstyr som gir klassifisering. Det er til nå blitt registrert med to ulike apparattyper.
...
Med utgangspunkt i NorSIKT-klassene er det i tabellene nedenfor satt opp hvilke klasser i de andre klassifiseringssystemene som er de tilsvarende. De siste kolonnene i tabellene angir med farge om det er en entydig sammenheng mellom klassene eller ikke, altså om det er en en-til-en-relasjon. Grønn farge indikerer entydig sammenheng med NorSIKT-klassen. 

Her ser vi bort fra at registrerte motorsykler med henger ikke kan skilles fra motorsykler uten henger. NorSIKT har egen klasse for motorsykkel med henger, men det er ikke en egen klasse for dette hos trafikkregistreringsutstyret. Motorsykler med henger er i de aller fleste tilfeller ganske fåtallig, så disse kan for praktiske formål slås sammen med klassen for motorsykler uten henger.


## NorSIKT nivå 2
Nivå 2 omfatter fem klasser og sammenheng med øvrige klassifiseringer er vist i @tbl-norsikt_2_compare.  For nivå 2 er det en helt entydig sammenheng med klassene som trafikkregistreringsutstyret for kontinuerlig registrering gir. Det samme gjelder for det mobile utstyret, bortsett fra at Armadillo ikke har egne klasser for kjøretøy med henger.

```{r}
#| label: tbl-norsikt_2_compare
#| tbl-cap: "Sammenheng mellom NorSIKT nivå 2 og andre klassifiseringssystemer."
...
Den eneste forskjellen mellom nivå 2 og 3 er at motorsykler og lignende (f.eks. mopedbil, ATV mm.) er skilt ut som egen klasse. 
...
Forskjellen fra nivå 3 er at lett og tung buss er skilt ut som egne klasser med og uten henger. Det er kun LM og TOPO som har egen klasse for buss, men da uten å spesifisere om det er med eller  uten henger. Andel tunge busser med henger vil være fåtallig slik at disse kan inngå i klassen for buss. En tilpasning av nivå 4 der utvidelsen fra nivå 3 kun er en egen klasse for tunge busser vil passe utstyret best.
...
Utvidelsen fra nivå 4 til nivå 5 består av at de tunge deles inn i egne klasser for henholdsvis lastebil og trekkbil med og uten henger, samt en egen klasse for øvrige tunge kjøretøy. Denne utvidelsen har ingen tilsvarende klasser fra EMU3 eller Armadillo, men LM og TOPO har egne klasser for lastebil med henger og trekkbil med semitrailer.
...
Basert på klassene som fås fra dagens trafikkregistreringsutstyr er det tre nivåer i NorSIKT som er aktuelle å benytte. Det er nivå 2, 3 og 4. Men ikke alle klasser har helt entydig sammenheng med utstyrsklassene. Ved å tilpasse klassene noe, kan vi få dette entydig.

NorSIKT nivå 2 og 3 kan benyttes som de er. Armadillo har ikke egne klasser med henger, men har sammenheng med de øvrige klassene. 

NorSIKT nivå 4 må tilpasses på flere måter. For det første må samme tilpasning som i nivå 3 gjøres, ved at motorsykler med og uten henger er en og samme klasse. Videre kan lette busser beholdes i klassen for lette kjøretøy slik de er i nivå 3. De tunge bussene kan skilles ut fra tunge, men da uten å skille på om det er med eller uten henger. I tillegg kan de tunge splittes opp delvis slik de gjøres i nivå 5, ved å ha en klasse for lastebil og trekkbil uten henger, og så en egen klasse for lastebil med henger og en klasse for trekkbil med semitrailer. Dette nivået er da aktuelt kun for LM og ikke EMU3, og sammenhengen er vist i @tbl-norsikt_4_mapping.
...
Kjøretøyforskriftens § 2-6 definerer kjøretøylengde som kjøretøyets største utvendige lengde fra forreste til bakerste punkt. Kjøretøylengden som er oppgitt i Kjøretøyregisteret er i noen tilfeller åpenbart feil. Disse er her luket ut etter følgende kriterier for forventet lengdeintervall:
...
M2 og M3 har bare troverdige lengder i datasettet. Antall kjøretøy i kjøretøygruppene er vist i @tbl-autosys.
...
#| fig-cap: "Fordeling av Kjøretøyregisterets lengder per kjøretøygruppe. Figurenes areal angir ikke størrelsesforhold mellom klassene."
#| fig-height: 6
...
#| fig-height: 6
#| fig-cap: "Fordeling av Kjøretøyregisterets tillatte totalvekter per kjøretøygruppe. Figurenes areal angir ikke størrelsesforhold mellom klassene."

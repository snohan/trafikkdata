...
traffic_links_wide <- trafikklenker_selected %>%
  dplyr::select(ID,
  ...
  tidyr::pivot_wider(
    names_from = with,
    names_prefix = "med_",
    values_from = pred_aadt
  ) %>% 
  dplyr::rowwise() %>% 
  dplyr::mutate(aadt = sum(med_0, med_1, na.rm = TRUE)) %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(across(.cols = med_1:aadt,
                       ~ round(.x)))
...
traffic_links_transformed <- links_selected_id %>% 
  ...
  dplyr::mutate(line_length = round(sf::st_length(geom),
                                    digits = 0)) %>% 
  dplyr::mutate(link_label = paste("ÅDT ", aadt, "<br/>",
                                   "Med ", med_1, "<br/>",
                                   "Mot ", med_0, "<br/>",
                                   line_length, " m"),

...
geopackage_file <- "Trafikklenker_20210216150826.gpkg"
...
link_query <- "SELECT * FROM \"TrafikkLenker\" WHERE roadref_category IS 'E' AND roadref_number IS 134"
...
trafikklenker_combined <- trafikklenker_selected %>%
  sf::st_zm(drop = T, what = "ZM") %>% 
  dplyr::select(ID, START_NODE_OID, END_NODE_OID, DIRECTION, COMMENT, ROADREF_CATEGORY,
                ROADREF_NUMBER, ROADREF_START, ROADREF_END) %>% 
  ...
  dplyr::mutate(line_length = round(sf::st_length(GEOMETRY),
                                    digits = 0)) %>% 
  dplyr::mutate(link_label = paste(ROADREF_START, ROADREF_END, line_length, sep = "<br/>"),

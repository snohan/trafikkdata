...
top_row_query <- "SELECT * FROM \"TrafikkLenker\" WHERE geometry IS NOT NULL AND min_funksjonell_vegklasse IS 1 LIMIT 5000"
...
# 
# lenker_uten_geometri_med_stedfesting <- DBI::dbSendQuery(databasekobling,
#                                                          sql_query_for_null_geometry)
# 
# lenker_uten_geometri_med_stedfesting_resultat <- DBI::dbFetch(lenker_uten_geometri_med_stedfesting)
# 
...
```{r intersections}
# Tilhørende kryss
unique_start_node_ids <- trafikklenker_top$START_NODE_OID %>% 
   unique()
unique_end_node_ids <- trafikklenker_top$END_NODE_OID %>% 
   unique()
...
```{r manipulate_links}
trafikklenker_combined <- trafikklenker_top %>%
  sf::st_zm(drop = T, what = "ZM") %>% 

...
source("H:/Programmering/byindeks/get_from_nvdb_api.R")
...
#element_query <- "SELECT * FROM \"Trafikklenker\" WHERE ELEMENT_ID IN (3112188, 3112207, 2997535, 2997537, 2997540, 2997539, 3112180, 3112181, 2997528, 2997531, 3112190, 3112189, 2997529, 3112179)"

element_query <- "SELECT feature_oid, start_node_oid, end_node_oid, geometry FROM \"Trafikklenker\" LIMIT 10000;"
...
#complete_links <- trafikklenker_element_clean %>% 
#  dplyr::group_by(FEATURE_OID) %>% 
#  dplyr::summarise(combined_geometry = sf::st_combine(GEOMETRY))
...
trafikklenker_kommune <- trafikklenker_element_clean %>% 
  dplyr::mutate(innafor = sf::st_intersects(trafikklenker_element_clean, kommune_polygon, sparse = FALSE))

trafikklenker_trondheim <- trafikklenker_kommune %>% 
  filter(innafor == TRUE)


# Tilhørende kryss, hmmm: TODO: outer intersections of combined links
...
leaflet(width = "100%",
  ...
  addTiles(urlTemplate = nvdb_map_url,
   ...
   addPolylines(data = trafikklenker_trondheim,
